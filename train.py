from datasets import load_citation
from utils import *
...
def train_pubmed(model, data, tau, optimizer, adj_label_1, adj_label_2, adj_label_3, features, alpha, idx_train):
    features_batch, adj_label_1_batch, adj_label_2_batch, adj_label_3_batch = get_batch(adj_label_1, adj_label_2, adj_label_3, idx_train, features, batch_size=2000)
    model.train()
    optimizer.zero_grad()
    output, x_1, x_2, x_3 = model(features_batch)
    loss_train_class = F.nll_loss(output[idx_train], data.y[idx_train])
    loss_Ncontrast_1 = Ncontrast(x_1, adj_label_1_batch, tau)
    loss_Ncontrast_2 = Ncontrast(x_2, adj_label_2_batch, tau)
    loss_Ncontrast_3 = Ncontrast(x_3, adj_label_3_batch, tau)
    loss_train = loss_train_class + alpha * (loss_Ncontrast_1 + loss_Ncontrast_2 + loss_Ncontrast_3)
    loss_train.backward()
    optimizer.step()
    return
...
def run(dataset, model, runs, epochs, lr, weight_decay, early_stopping, alpha, tau, k, permute_masks=None, lcc=False, adj_label_1=None, adj_label_2=None,
    ...
    for _ in pbar:
        ...
        for epoch in range(1, epochs + 1):
            if k == 2:
                train_two_layer_model(model, optimizer, data, adj_label_1, adj_label_2, alpha, tau)
            elif k == 3:
                if dataset == 'PubMed':
                    adj, features, labels, idx_train, idx_val, idx_test = load_citation('pubmed', 'AugNormAdj', True)
                    train_pubmed(model, data, tau, optimizer, adj_label_1, adj_label_2, adj_label_3, features, alpha, idx_train)
                else:
                    train_three_layer_model(model, optimizer, data, adj_label_1, adj_label_2, adj_label_3, alpha, tau)
            else:
                train_four_layer_model(model, optimizer, data, adj_label_1, adj_label_2, adj_label_3, adj_label_4,
                                             alpha, tau)
